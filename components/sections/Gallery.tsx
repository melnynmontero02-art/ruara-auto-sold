...
export default function Gallery() {
  ...
  return (
    <section id="galeria" style={{ background:'#FFFFFF', padding:'96px 0' }}>
      <div className="max-w-7xl mx-auto px-6">
        <div ref={ref} className="text-center mb-14">
          <motion.div initial={{ opacity:0,y:14 }} animate={inView?{opacity:1,y:0}:{}} transition={{ duration:0.6 }}
            className="section-label mb-5 mx-auto w-fit"><Images size={10}/>Nuestra flota
          </motion.div>
        ...
        <motion.div initial={{ opacity:0 }} animate={inView?{opacity:1}:{}} transition={{ duration:0.8,delay:0.2 }}
          ...
          {galleryImages.map((img,i) => (
            <motion.div key={img.id}
              ...
              <div className="relative w-full h-full" style={{ minHeight:'inherit',background:'#F1F5F9' }}>
                {!errs[img.id] ? (
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(201,163,82,0.25)" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center"
                  ...
                  <div className="w-11 h-11 rounded-full flex items-center justify-center"
                    style={{ background:'#C9A352', boxShadow:'0 0 20px rgba(201,163,82,0.4)' }}>
                    <ZoomIn size={16} color="#FFF"/>
                  </div>
                </div>
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none rounded-xl"
                  style={{ border:'2px solid rgba(201,163,82,0.35)' }}/>
              </div>
            </motion.div>
          ))}
        </motion.div>
